...
# Load or initialize EVALUATION_SCHEMA
...
# Load or initialize PROMPT_TEMPLATES
...
if page == "介绍":
    st.title("学生评价系统介绍")
    st.markdown("""
    学生评价系统：旨在帮助教师评估学生的学习水平和状态，并生成结构化的反馈报告。
    ...
    """)
        ...
        st.markdown("""
        **（2）初始时学生（除学号1，张伟以外）均没有分数，此时需要先填写问卷才能查看学生能力展示以及生成反馈报告；对学生重新打分后，生成的报告更加注重于描述该学生两轮分数前后的进步**
    """)

        st.markdown("""
        **（3）对学生重新打分后，生成的报告更加注重于描述该学生两轮分数前后的进步**
    """)
